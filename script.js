...
var pauseGame = false;
var endGame = false
var recentDirection = SNAKE_DIRECTION_RIGHT
...
const game = {
    ...
    move(event) {
        ...
        switch (event.keyCode) {
            case 38:
                ...
                recentDirection = SNAKE_DIRECTION_UP;
                break;
            case 40:
                direction = SNAKE_DIRECTION_DOWN;
                recentDirection = SNAKE_DIRECTION_DOWN;
                break;
            case 37:
                direction = SNAKE_DIRECTION_LEFT;
                recentDirection = SNAKE_DIRECTION_LEFT;
                break;
            case 39:
                direction = SNAKE_DIRECTION_RIGHT;
                recentDirection = SNAKE_DIRECTION_RIGHT;
                break;
            default:
                return;
        }
    ...
    Score() {
        ...
        var initialAccount = parseInt(score.textContent)
        initialAccount = initialAccount + 10
        score.textContent = initialAccount

        if (initialAccount % 100 == 0 && speed != 0) {
            speed = speed - 50
        }
    },
...
const food = {
    ...
    generateItem() {
        ...
        var mergedArrayOfCoordinates = this.items.concat(snake.parts)
        var i = 0;
        while (mergedArrayOfCoordinates.length > i) {
            if (newItem.top == mergedArrayOfCoordinates[i].top && newItem.left == mergedArrayOfCoordinates[i].left) {
...
function interval() {
    var work = true;
    if (startGame && !endGame && !pauseGame) {
        snake.render();
        food.render();
        var direction = recentDirection

        snake.setDirection(direction);
        const nextPosition = snake.getNextPosition();

        for (var i = 0; i < snake.parts.length; i++) {
            if (nextPosition.top == snake.parts[i].top && nextPosition.left == snake.parts[i].left) {
                clearInterval(intervalID);
                work = false
                game.stop()
                break
